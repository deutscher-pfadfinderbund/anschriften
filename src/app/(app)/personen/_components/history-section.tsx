...
import { HOVER_REVEAL, TOUCH_HIT, cn } from "@/lib/utils";
  ...
  return (
    <div className="mt-4 border-t border-line pt-3">
      <button
        ...
        className={cn(
          TOUCH_HIT,
          "flex w-full items-center gap-1.5 text-[13px] font-semibold text-ink-soft transition-colors hover:text-ink",
        )}
      >
        ...
        <div className="mt-2.5 flex flex-col gap-1.5">
          {historical.length === 0 ? (
            ...
            historical.map((h) => (
              <div
                key={h.id}
                className="group/row flex items-center gap-2 rounded-md border border-line bg-surface-2 px-2.5 py-1.5 text-[13px]"
              >
          ...
          <button
            ...
            className={cn(
              TOUCH_HIT,
              "mt-1 inline-flex items-center gap-1 self-start text-[13px] text-ink-soft transition-colors hover:text-fir disabled:opacity-50",
            )}
          >
